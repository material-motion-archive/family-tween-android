 ...
 */
public abstract class BaseTweenPerformer<T> extends Performer<T> implements ContinuousPerforming {

  protected BaseTweenPerformer() {
  }
  ...
  public void addPlan(Plan<T> plan) {
    if (plan instanceof BaseTween) {
      addTween((BaseTween<T, ?>) plan);
    } else {
      throw new IllegalArgumentException("Plan type not supported for " + plan);
    }
  }

  private void addTween(BaseTween<T, ?> plan) {
    if (!validate(plan)) {
  ...
  private boolean validate(BaseTween<T, ?> plan) {
  ...
  private PropertyValuesHolder createPropertyValuesHolder(BaseTween<T, ?> plan) {
    // Create keyframes.
    Keyframe[] keyframes;
